...
def clean_gadget(gadget):
    ...
    # regular expression to catch single-line comment
    rx_comment = re.compile("//.*")
    # regular expression to find JavaScript function name candidates
    rx_fun = re.compile(r"\bfunction\s+([_$A-Za-z][\w$]*)\b")
    ...
    for line in gadget:
        ...
        ascii_line = rx_comment.sub("", line)
        ...
        for fun_name in user_fun:
            # Check if the function name is not a keyword
            if fun_name not in keywords:
                # check to see if function name already in dictionary
                if fun_name not in fun_symbols.keys():
                    fun_symbols[fun_name] = "FUN" + str(fun_count)
                    fun_count += 1
                # replace the function name with its symbol
                ascii_line = re.sub(
                    r"\b(" + fun_name + r")\b", fun_symbols[fun_name], ascii_line
                )

        for var_name in user_var:
            # Check if the variable name is not a keyword
            if var_name not in keywords:
                # check to see if variable name already in dictionary
                if var_name not in var_symbols.keys():
                    var_symbols[var_name] = "VAR" + str(var_count)
                    var_count += 1
                # replace the variable name with its symbol
                ascii_line = re.sub(
                    r"\b(" + var_name + r")\b", var_symbols[var_name], ascii_line
                )

        cleaned_gadget.append(ascii_line.strip())
