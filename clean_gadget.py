...
# keywords up to ECMAScript 2021; immutable set
keywords = frozenset(
    [
        "break",
        "case",
...
def clean_gadget(gadget):
    ...
    for line in gadget:
        # remove comments
        line = re.sub(rx_comment, "", line)
        # replace function names
        line = re.sub(
            rx_fun,
            lambda match: "function "
            + (
                fun_symbols[match.group(1)]
                if match.group(1) in fun_symbols
                else match.group(1)
            ),
            line,
        )
        ...
        line = re.sub(
            ...
            lambda match: "var "
            + (
                var_symbols[match.group(1)]
                if match.group(1) in var_symbols
                else match.group(1)
            ),
            line,
        )
